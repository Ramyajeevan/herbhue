  ...
  <div class="col-12">
    ...
    <div class="card">
      <div class="border-bottom title-part-padding">
        <h4 class="card-title mb-0">Personalise List</h4>
      </div>
      <div class="card-body">
        <div class="table-responsive">
          <table id="file_export" class="table table-striped table-bordered display text-nowrap">
            ...
            <tbody>
              @if(count($personalise)>0)
              @foreach ($personalise as $personal)
                <tr>
                  <td>{{ ($loop->index)+1 }}</td>
                  <td>{{ $personal->user_name }}</td>
                  <td>{{ $personal->question }}</td>
                  <td>{{ $personal->answer }}</td>
                  <td>
                    <a href="{{route('personalise.edit',$personal->id)}}"  class="btn btn-sm text-primary border-primary" data-original-title="Edit Personalise">
                      <i data-feather="edit-3"></i>
                    </a>
                    <a href="javascript:void(0);" onclick="deletepersonalise({{$personal->id}})" class="btn btn-sm text-danger border-danger" data-original-title="Delete">
              ...
              @else
              <tr>
                  <td colspan='5' align="center">No Personalise Found</td>
              </tr>
              @endif
            </tbody>
...
<script>
  function deletepersonalise(id)
  {
    if(confirm("Are you sure want to delete this personalise?"))
    {
      var url="{{URL('personalise')}}/"+id;
        $.ajax(
        {
            url: url,
            method: 'delete', 
